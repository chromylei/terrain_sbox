...
#include "lightmap.afx.h"
#define EFFECT_GEN_DIR "out/dbg/gen/tersbox/fter/lightmap/"
#define SHADER_NAME "lightmap.afx"
...
#define LIGHTMAP_PATH FILE_PATH_LITERAL("tersbox/fter/res/tex/lightmap.jpg")
using base::FilePath;
...
class MainDelegate : public azer::WindowHost::Delegate {
 ...
 private:
  ...
  azer::TexturePtr lightmap_tex_;
  std::unique_ptr<LightmapEffect> effect_;
  LightmapEffect::DirLight light_;
...
void MainDelegate::InitPhysicsBuffer(azer::RenderSystem* rs) {
  ...
  LightmapEffect::Vertex* vertex = (LightmapEffect::Vertex*)vdata->pointer();
  float cell = 1.0f / (float)tile_.GetGridLineNum();
  cnt = 0;
  for (int i = 0; i < tile_.GetGridLineNum(); ++i) {
    for (int j = 0; j < tile_.GetGridLineNum(); ++j) {
      LightmapEffect::Vertex* v = vertex + cnt;
...
void MainDelegate::OnRenderScene(double time, float delta_time) {
  ...
  effect_->SetLightmapTex(lightmap_tex_);
